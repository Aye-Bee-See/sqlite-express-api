...
| `ENCRYPTION_KEY_PREVIOUS`                     | No       | none                                      | Only while changing `ENCRYPTION_KEY`: the old key, so letters wrapped with it stay readable until `npm run encryption:rekey` has moved them. See [Encryption](#encryption). |
