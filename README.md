...
| `PEN_NAME_NEW_PER_YEAR`                       | No       | `2`                                       | Brand-new pen names an account may take in a rolling year. Going back to one of its own old names does not count.                                                                                       |
...
| `ENCRYPTION_MODE`                             | No       | `e2e`                                     | How letters are encrypted: `e2e` (readers hold the keys) or `server` (the API can read letters; for older databases). See [Encryption](#encryption).                                                    |
...
- Generate the key once with `npm run keygen`, put it in `.env`, and back it up somewhere other than the server. Server mode needs it for migrations, seeds, and every boot; end-to-end mode needs it only while letters written in server mode still wait for a reader.
...
| `penName`                  | The site-unique name letters are signed with; see [Pen names](#pen-names). Optional in the API (clients ask for it at sign-up), 3 to 40 characters, changeable within the limits there; old names are kept for ever.                                                             |
...
- `GET /auth/pen-name` (any signed-in account) answers `{ penName, names: [{ name, current, since }] }`, current first, together with what the limits leave: `changeAllowedAt` (null before the first name is chosen), `newNamesLeft`, `newNamesWindowEnds` (when a spent new name comes back, null when none is spent), and the two settings in force, `cooldownDays` and `newPerYear`.

**Changes are limited**, because a name once used is never released: an account renaming itself in a loop would empty a namespace everybody shares, and a name that changes every month loses the replies addressed to the last one. Two limits, both settable per deployment ([Configure](#configure)):
