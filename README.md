...
- **Nobody can make an account for someone else with `POST /auth/user`.** A split account's auth key and keys are made on its holder's device, which an admin creating a `chapter` account does not have. Under the flag, people get accounts through the flows built for that: an [invite code](#invite-codes) (writers), an [invitation](#invitations) of kind `member` (group admins) or `group`, or a [managed writer](#managed-writers) with a claim token. Seed scripts that create accounts directly run before the flag goes on, or with it off.
- On a server-mode API a split account is made with `kdfSalt` and `kdfParams` alone (above); the flag does not require keys.
