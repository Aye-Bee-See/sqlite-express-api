...
Returns `201` with the submission. Omit `target` to propose a brand-new record. Either way the `fields` are run through the resource's own validation at filing, without saving anything, so a submitter hears about a problem immediately as a `400` with an `errors` array: a new record is checked whole (a prison needs `prisonName` and `address`, a group `name` and `location`), and an edit has its proposed fields checked against the target (a prisoner `status` of `flying` or an unknown facility mail rule tag is refused, and so is `photoLimit` for a facility whose stored `mailRules` include `no_photos`, or that tag for a facility with a stored `photoLimit`). A reviewer-only field, an unknown resource, empty `fields`, or a `target` that does not exist (or that the caller cannot see: non-staff may only propose changes to published records) are `400` or `404` at filing time. Checks that need the database, such as a `prison` id on a new prisoner, happen when the proposal is approved.
