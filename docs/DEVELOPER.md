...
- **Writes.** `createPrison` creates the row (validation resolves the tags first), sets the links, and returns a fresh read. `createBulkPrisons` (seeds) goes through it one facility at a time. `updatePrison` splits `mailRules` from the columns; when rules or `photoLimit` are involved it runs inside `oneRuleWriteAtATime` (a `createSerialQueue()` from `services/serial.js`, the same helper behind the group key lock): resolve the tags (a retired rule may stay if the facility already has it, never arrive), check the photo rule against whichever half was not sent, write the columns, then `setMail_rule_details`. It returns `[1]` or `[0]` like `Model.update`, so the controller's 404 handling is unchanged. The columns update is given the new tags as the virtual field so the model-level `photoRules` validator sees the rules as they will be.
