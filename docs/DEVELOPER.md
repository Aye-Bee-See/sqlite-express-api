...
- **News feed** (`services/news-feed.js`, `database/models/news-item.model.js`, migration `2026.09.25T00.00.00.news-items.js`). A dependency-free RSS 2.0 reader (`parseRss`, `plainText`, `summarize`) and `fetchFeed` with a 15-second timeout, a size cap, and an honest user agent; `NewsItem.refresh` upserts the newest `newsFeed.keep` items by `guid` and prunes the rest, `latest` serves `GET /news`, and `schedule` (called from `database/sql-database.js` after the database is ready) pulls in the background and on an unref'd interval. Off unless `NEWS_FEED_URL` is set; the test helpers pin it empty, and `test/news.test.js` sets one and hands `refresh` its own `fetch`, so no test touches the network.
- **Pen names and the reply reference** (decided 22 September 2026; migration `2026.09.24T01.00.00.pen-names-reply-reference.js`, which also backfills a reference onto every existing outgoing letter and adds the `no_reference_numbers` and `reply_sheet_allowed` rules). `PenName` (`database/models/pen-name.model.js`) holds every name ever used, keyed by `nameKey` (NFKC, lower case, one space) with a unique index; `claim(userId, name)` retires the current row and inserts or un-retires, and the `User.penName` column is only ever written beside it (`User.takePenName`, `User.renamePen`, or inside the join and claim transactions). Changes are limited (migration `2026.09.26T00.00.00.pen-name-limits.js` adds `PenNames.claimedAt`, when a name last became current, which differs from `createdAt` for a name taken back): `claim` with `enforce` calls `refuseOverLimit` **inside the same transaction**, so two renames at once cannot both spend the last new name of the year, and `changeStatus` answers the same numbers for `GET /auth/pen-name` before anyone types. `UserController.update` enforces for the account itself and passes `enforce: false` for a superadmin or a group editing its own unclaimed writer, auditing that as `user.penName`; it also checks before the row is written, so a refusal changes nothing. Tests that need a second name call `agePenNames` (test/helpers.js) rather than waiting out the cooldown. `ReplyReference` (`database/models/reply-reference.model.js`) is issued in `Message.#finishLetter` for every non-reply (`database/reply-reference.js`: nine digits, Luhn check digit); `markMailed` sets `mailedAt`/`expiresAt` and pins `chapter` to the group that mailed it, inside the status transaction; `purge` in the retention run removes rows whose letter is gone and whose time is up. `MessageController.#referenceFor` is the one place that decides whether a caller may see a reference (the letter's current relay group while it exists, the row's `chapter` after), and `create` uses it when a reply carries `reference`. `attachPrintDetails` adds `footer` (name, care-of, the formatted reference or null under `no_reference_numbers`, `replySheetAllowed`).
- **Directory writes are admin-only** (decided 22 September 2026): `POST`/`PUT`/`DELETE` on `/prison/prison` and `/prisoner/prisoner` carry `requireRole(ADMIN)`; a chapter's edit is a proposal (`POST /moderation/submission`, `requireRole(ADMIN, CHAPTER)` plus an active-group check in `ModerationController.create`, so a pending group gets `groupRefusal`). The link endpoints (`/prison/relay`, `/prisoner/support`) stay open to chapter accounts through `#ownGroupOnly` in each controller: own active group, else `groupRefusal` or a 403. Tests that used to write the directory as `f.chapter` now use `f.admin`; `test/moderation.test.js` makes two chapter-role submitters in two active groups.
