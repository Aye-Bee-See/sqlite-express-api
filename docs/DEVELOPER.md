...
- **Transactions.** `inTransaction(sequelize, work)` in `services/serial.js` runs one transaction at a time in the process, because an in-memory database has a single connection and a second `BEGIN` on it fails. Group key rotation uses it too. Take your own queue first and this one second, always in that order. On a file database a transaction has its own connection and other requests' writes wait for it (Sequelize retries `SQLITE_BUSY`); these transactions are a handful of statements.
