...
**Never `queryInterface.removeColumn` or `changeColumn`.** On SQLite Sequelize rebuilds the table from `describeTable()`, which knows nothing of `ON DELETE` / `ON UPDATE` rules, `AUTOINCREMENT`, or indexes, and all three are gone afterwards. That happened to `Messages` and `Prisons`: deleting a relay group failed on the foreign key instead of setting `relayChapter` to `NULL`, and the id of a deleted newest letter was given to the next one (which an `Idempotency-Key`, a notification, or an audit entry would then point at). `2026.09.20T01.00.00.repair-rebuilt-tables.js` rebuilt both from their own `CREATE TABLE` text with the rules put back. Drop a column with `dropColumn()` from `database/migration-helpers.js` (SQLite's own `ALTER TABLE ... DROP COLUMN`, which rebuilds nothing; drop the column's index first). The migrations test fails if any table has a `REFERENCES` without an `ON DELETE`, or an `id` without `AUTOINCREMENT`.
...
- **Busy timeout** (`BUSY_TIMEOUT_MS`, 5 s), set on every connection as it is opened. Sequelize opens a connection per transaction, and without a timeout a second writer failed at once with `SQLITE_BUSY`.
