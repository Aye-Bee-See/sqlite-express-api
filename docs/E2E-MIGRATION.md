...
A checklist for moving a running letters.support API from `ENCRYPTION_MODE=server` to `ENCRYPTION_MODE=e2e`. Read it all once before starting; the steps are in order, and the last one is the only one that cannot be undone.
