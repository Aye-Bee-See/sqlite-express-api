...
export default class UserController extends RouteController {
	...
	#handlePass(res, user, type) {
		if (user) {
			...
			const errorVal = new Error();
			this.#handleErr(res, errorVal, type);
		}
	}
	...
	#handleUsers(res, users) {
		...
		if (users.length > 0) {
			...
			const errorVal = new Error();
			this.#handleErr(res, errorVal, 'empty');
		}
	}
	...
	async getMany(req, res) {
		...
		if (role) {
			...
			const errorVal= !(err instanceof Error) ? new Error(err) : err;
			this.#handleErr(res, errorVal, 'role');
			}
		} else {
			try {
		...
		switch (type) {
			case 'id':
				try {
					...
					errorVal = !(err instanceof Error) ? new Error(err) : err;
