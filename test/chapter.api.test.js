...
describe('Chapters API', function() {
                                ...
                                request(app)
                                    ...
                                    .end((err, res) => {
                                        ...
                                        if (!res.body.data || !res.body.data.id) {
                                            console.error("User creation response missing data for chapter tests:", res.body);
                                        ...
                                        request(app)
                                            ...
                                            .end((err, res) => {
                                                if (err) {
                                                    console.error("Error logging in user for chapter tests:", res ? res.body : err);
                                                    return done(err);
                                                }
                                                if (!res.body.data || !res.body.data.token || !res.body.data.token.token) {
                                                    console.error("Login response missing token for chapter tests:", res.body);
                                                    return done(new Error('Auth token not received for chapter tests'));
                                                }
                                                authToken = res.body.data.token.token;
                                                
                                                // 5. Start the server
                                                server = app.listen(4004, () => { // Use a different port
    ...
    after(function(done) {
        ...
        function checkDone() {
            tasksCompleted++;
            if (tasksCompleted >= totalTasks) { // Use >= to be safe
                done();
            }
        }
        ...
        } else if (cleanupTasks.length === 0) { // If no server and no DB tasks
             done();
        } else if (tasksCompleted === cleanupTasks.length && !server) { // All DB tasks done, no server
            done();
        }
    });
        ...
        it('should return all chapters', function(done) {
            request(app)
                ...
                .end((err, res) => {
                    ...
                    if (createdChapterId) { // Only check if a chapter was successfully created
        ...
        it('should update an existing chapter successfully', function(done) {
            ...
            request(app)
                ...
                .end((err, res) => {
                    ...
                    expect(JSON.parse(res.body.data.newChapter.location)).to.deep.equal(JSON.parse(updatedChapter.location));
