...
describe('Prisons API', function () {
									...
									.end((err, res) => {
										...
										request(app)
											...
											.end((err, res) => {
												if (err) {
													console.error(
														'Error logging in user for prison tests:',
														res ? res.body : err
													);
													return done(err);
												}
	...
	after(function (done) {
		...
		if (server) {
			...
			// Handle case where server might not have started
			done();
		}
	});
			...
			request(app)
				...
				.end((err, res) => {
					...
					expect(res.body.errors)
						.to.be.an('array')
						.that.includes('Prison.prisonName cannot be null');
					done();
				});
		});
		...
		it('should update an existing prison successfully', function (done) {
			...
			request(app)
				...
				.end((err, res) => {
					...
					expect(JSON.parse(res.body.data.newPrison.address)).to.deep.equal(
						JSON.parse(updatedPrison.address)
					);
					done();
				});
		});
